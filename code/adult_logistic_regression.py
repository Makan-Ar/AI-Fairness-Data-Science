...
param_grid = [{'penalty': ['l2'], 'C': [0.01, 0.1, 1, 5, 10, 100], 'tol': [1e-2, 1e-3, 1e-4, 1e-7, 1e-8],
               'solver': ['lbfgs', 'liblinear', 'sag', 'saga']},
              {'penalty': ['l1'], 'C': [0.01, 0.1, 1, 5, 10, 100], 'tol': [1e-2, 1e-3, 1e-4, 1e-7, 1e-8],
               'solver': ['liblinear', 'saga']}]


clf = make_pipeline(StandardScaler(), GridSearchCV(LogisticRegression(), param_grid=param_grid, n_jobs=25, cv=5, refit=True))
clf.fit(adult_data, adult_targets)
print("Best parameters set found on development set:\n")
c = clf.named_steps.gridsearchcv.best_params_
print(c)

# Training the classifier -- This gets an accuracy of
